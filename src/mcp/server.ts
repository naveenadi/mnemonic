...
import { createContext, resolveLLM, destroyContext, CliContext } from '../cli/context-factory.js';
import { handleGet, handleMultiGet } from '../cli/handlers/document.js';
import type { SearchOptions } from '../types.js';

let mcpCtx: CliContext | null = null;

export async function startMcpServer(dbPath: string, args: string[]): Promise<void> {
  // Initialize shared CliContext
  const ctx = createContext(dbPath, false);
  await ctx.db.loadVectors();

  // Resolve LLM (non-blocking — will use available backend)
  const llm = await resolveLLM(false);
  if (llm) {
    ctx.llm = llm;
  ...
  const httpPort = getArg(args, '--http') ? parseInt(getArg(args, '--port') ?? '8181', 10) : undefined;
...
- lineNumbers: Prefix lines with numbers (default true).`,
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    const { name, arguments: mcpArgs } = request.params;

    try {
      switch (name) {
        case 'query':
          return handleMcpQuery(mcpArgs);
        case 'get':
          return handleMcpGet(mcpArgs);
        case 'multi_get':
          return handleMcpMultiGet(mcpArgs);
        case 'status':
...
async function handleMcpQuery(args: any) {
...
async function handleMcpGet(args: any) {
  if (!mcpCtx) throw new Error('MCP context not initialized');

  const { file, fromLine, maxLines, lineNumbers } = args || {};
  if (!file) throw new Error('file parameter is required');

  try {
    const result = handleGet(mcpCtx, {
      identifier: file,
      fromLine,
      maxLines,
    ...
    let output = `mne://${result.meta.collection}/${result.meta.path}  #${result.meta.docid}\n---\n`;
...
async function handleMcpMultiGet(args: any) {
  if (!mcpCtx) throw new Error('MCP context not initialized');

  const { pattern, maxLines, lineNumbers } = args || {};
  if (!pattern) throw new Error('pattern parameter is required');

  const output = handleMultiGet(mcpCtx, {
    pattern,
